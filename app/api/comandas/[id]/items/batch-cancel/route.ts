...
export async function POST(request: NextRequest, { params }: { params: { id: string } }) {
  ...
  // Regla: no se cancelan productos de una cuenta impresa / por cobrar sin reabrirla primero.
  ...
  if (sentItems.length > 0) {
    void notify({ roles: ["MANAGER"], type: "audit", title: "Productos cancelados", body: `${comanda.folio} · ${sentItems.length} enviado(s)${reason ? ` · ${reason}` : ""}`, url: "/admin/comandas" });
  }
