...
export function PlantaAltaMap({ tables, pairs, triples, quads, guests, selection, onSelect, renderTable }: Props) {
  ...
  return (
    <div style={{ width: "100%", aspectRatio: "16/7", position: "relative", background: "var(--sl-panel)", borderRadius: 14, border: "1px solid rgb(var(--sl-gold-rgb) / 0.35)", overflow: "hidden", userSelect: "none" }}>
      ...
      <div style={{ position: "absolute", top: 10, left: 16 }}>
        ...
        <div style={{ fontSize: "clamp(0.5rem,1.2vw,0.7rem)", color: "rgb(var(--sl-cream-rgb) / 0.4)", letterSpacing: "0.12em", marginTop: 2 }}>PLANTA ALTA</div>
      </div>

      {/* ── ENTRADA ── */}
      <div style={{ position: "absolute", left: 0, top: "38%", width: "8%", height: "16%", background: "rgb(var(--sl-veil-rgb) / 0.04)", borderRadius: "0 6px 6px 0", border: "1px solid rgb(var(--sl-veil-rgb) / 0.07)", borderLeft: "none", display: "flex", alignItems: "center", justifyContent: "center" }}>
        <span style={{ fontSize: "0.42rem", color: "rgb(var(--sl-veil-rgb) / 0.25)", letterSpacing: "0.1em" }}>ENTRADA</span>
      </div>

      {/* ── Salón 1 ── */}
      <div style={{ position: "absolute", right: "2%", top: "10%", width: "46%", height: "80%", background: "var(--sl-panel)", border: "1px solid rgb(var(--sl-veil-rgb) / 0.09)", borderRadius: 10, display: "flex", alignItems: "center", justifyContent: "center" }}>
