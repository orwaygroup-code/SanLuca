...
export function PrivadoSelect({ tables, guests, selection, onSelect }: Props) {
  ...
  if (!table) return (
    <div style={{ textAlign: "center", padding: "2rem", color: "rgb(var(--sl-cream-rgb) / 0.5)" }}>
    ...
    <div style={{ display: "flex", flexDirection: "column", alignItems: "center", gap: 20, padding: "2rem 1rem" }}>
      ...
      <button
        ...
        style={{
          ...
          background:   isSelected  ? "rgb(var(--sl-gold-rgb) / 0.4)" : isAvailable ? "rgb(var(--sl-gold-rgb) / 0.1)" : "rgb(var(--sl-veil-rgb) / 0.03)",
          border:       `2px solid ${isSelected ? "var(--sl-gold)" : isAvailable ? "rgb(var(--sl-gold-rgb) / 0.5)" : "rgb(var(--sl-veil-rgb) / 0.1)"}`,
          borderRadius: 16,
          color:        isAvailable ? "var(--sl-cream)" : "rgb(var(--sl-veil-rgb) / 0.2)",
      ...
      >
        ...
        {!isAvailable && guests > table.capacity && (
          <span style={{ fontSize: "0.65rem", color: "var(--sl-danger)", marginTop: 4 }}>Máximo {table.capacity} personas</span>
        )}
      </button>

      <p style={{ fontSize: "0.8rem", color: "rgb(var(--sl-cream-rgb) / 0.5)", textAlign: "center", maxWidth: 280 }}>
        {isAvailable
          ? isSelected
